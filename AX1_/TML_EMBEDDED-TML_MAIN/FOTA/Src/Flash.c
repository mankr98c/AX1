...
void HAL_FLASH_Write_Buff(uint32_t FlashAddress, uint8_t *Data,
	...
	for (int l = 0; l < LoopCnt; l++) {
		...
		// Reverse the byte order of the 64-bit FlashData[j] variable
//		FlashData[0] = reverseByteOrder(FlashData[0]);
//		FlashData[1] = reverseByteOrder(FlashData[1]);
...
/*
 ...
 uint32_t a = (uint32_t) (input & 0xFFFFFFFF);
 uint32_t b = (uint32_t) (input >> 32);
 uint64_t merged = ((uint64_t) a << 32) | b;
...
void Erase_sectors(uint32_t FlashAddress) {
	...
	if (FlashAddress == L2) {
		end_sector = start_sector_address + (32 * FLASH_SECTOR_SIZE); // 16 sectors
...
//void WriteF_AppLoc(int LOC_change) {
//	if (LOC_change == 1) {
//
//		HAL_FLASH_Unlock();
//		uint64_t ADDrK = L1;
//		HAL_FLASH_Program(FLASH_TYPEPROGRAM_QUADWORD, App1_LOC_Addr, ADDrK);
//		HAL_FLASH_Lock();
//
//		HAL_FLASH_Unlock();
//		ADDrK = L2;
//		HAL_FLASH_Program(FLASH_TYPEPROGRAM_QUADWORD, App2_LOC_Addr, ADDrK);
//		HAL_FLASH_Lock();
//
//		ADDrK = L3;
//		HAL_FLASH_Unlock();
//		HAL_FLASH_Program(FLASH_TYPEPROGRAM_QUADWORD, App3_LOC_Addr, ADDrK);
//		HAL_FLASH_Lock();
//	}
//
//}

void writeF_CP(uint64_t Data) {
	HAL_FLASH_ERASE(Current_App_LOC);
	uint32_t KAddr = Current_App_LOC;
	uint64_t DT1 = Data - 1;
	HAL_FLASH_Unlock();
	HAL_FLASH_Program(FLASH_TYPEPROGRAM_QUADWORD, KAddr, &Data);
	HAL_FLASH_Program(FLASH_TYPEPROGRAM_QUADWORD, KAddr+16,&DT1);
	HAL_FLASH_Lock();
}
