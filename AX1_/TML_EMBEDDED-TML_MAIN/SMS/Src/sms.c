...
//    if (!EC200_Transmit("ATD9857821331;\r\n", sms_resp_buff, 2000)) { return false; }
    ...
    if (strstr(sms_resp_buff, "OK") == NULL) { return false; }
    // 2. SMS Notifications
...
bool SMS_Send(char *number,char *message){
	...
	if (!EC200_Transmit(cmd, sms_resp_buff, 2000)) { return false; }
	    uint8_t ctrlz[10]={};
	    sprintf(ctrlz,"%s\r\n%c",message,26);
	if(!EC200_Transmit(ctrlz, sms_resp_buff, 2000)) { return false; }
	return true;
}
void onMessage(){
...
bool SMS_Delete(int index) {
    ...
    if (!EC200_Transmit(command, sms_resp_buff, 2000)) { return false; }
    if (strstr(sms_resp_buff, "OK") == NULL) { return false; }
    return true;
}
